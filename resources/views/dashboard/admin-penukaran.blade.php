    ...
    <div class="py-8">
        <div class="content-wrap space-y-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold">Penukaran & Konfirmasi</h1>
                    <p class="section-subtitle">Kelola penukaran kode tunai, konfirmasi transfer, dan penukaran saldo penjual</p>
                </div>
                <a href="{{ route('dashboard.admin', [], false) }}" class="btn-secondary btn-sm">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                    Kembali
                </a>
            </div>

            <!-- Penukaran Kode Tunai + Konfirmasi Transfer -->
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div class="lg:col-span-2 panel-section">
                    ...
                    <form method="POST" action="{{ route('dashboard.admin.topups.tunai', [], false) }}" class="space-y-4"
                          x-data="{
                              kode: '',
                              uang: 0,
                              found: null,
                              ...
                              lookup() {
                                  ...
                                  if (val.length < 4) { this.found = null; this.jumlah = 0; this.userName = ''; this.error = false; return; }
                                  this.loading = true;
                                  this.error = false;
                                  this.debounceTimer = setTimeout(() => {
                                      fetch('{{ route('dashboard.admin.topups.lookup', [], false) }}?kode=' + encodeURIComponent(val), { credentials: 'same-origin' })
                        ...
                        <div>
                            ...
                            <template x-if="error">
                                <p class="text-xs mt-1 text-red-600 font-medium">Gagal menghubungi server. Coba lagi.</p>
                            </template>
                        </div>
                        <div>
                            ...
                            <tbody>
                                @forelse ($pendingTransfers as $topup)
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" action="{{ route('dashboard.admin.topups.confirm', $topup, false) }}" x-data x-on:submit.prevent="$dispatch('confirm-action', { title: 'Konfirmasi Top Up', message: 'Konfirmasi top up transfer Rp {{ number_format($topup->jumlah, 0, ',', '.') }} untuk {{ $topup->user->name }}?', type: 'confirm', confirmText: 'Ya, Konfirmasi', form: $el })">
                                                @csrf
                                                @method('PATCH')
            ...
            <div class="panel-section">
                ...
                <p class="text-sm mb-4" style="color: var(--muted);">Masukkan kode penukaran dari penjual. Setelah dikonfirmasi, serahkan uang tunai sesuai jumlah (sudah dipotong 10% untuk sekolah).</p>
                <form method="POST" action="{{ route('dashboard.admin.withdrawals.confirm', [], false) }}" class="space-y-4"
                      x-data="{
                          kode: '',
                          found: null,
                          jumlah: 0,
                          ...
                          lookup() {
                              clearTimeout(this.debounceTimer);
                              const val = this.kode.trim().toUpperCase();
                              if (val.length < 4) { this.found = null; this.jumlah = 0; this.potongan = 0; this.diterima = 0; this.userName = ''; this.error = false; return; }
                              this.loading = true;
                              this.error = false;
                              this.debounceTimer = setTimeout(() => {
                                  fetch('{{ route('dashboard.admin.withdrawals.lookup', [], false) }}?kode=' + encodeURIComponent(val), { credentials: 'same-origin' })
                                      .then(r => { if (!r.ok) throw new Error(r.status); return r.json(); })
                                      .then(d => {
                                          this.loading = false;
                                          this.found = d.found;
                                          if (d.found) {
                                              this.jumlah = d.jumlah;
                                              this.potongan = d.potongan;
                                              this.diterima = d.diterima;
                                              this.userName = d.user;
                                          } else {
                                              this.jumlah = 0;
                                              this.potongan = 0;
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                        <div class="sm:col-span-2">
                            ...
                            <template x-if="!loading && found === true">
                                <p class="text-xs mt-1 text-green-600 font-medium">
                                    Ditemukan &mdash; <span x-text="userName"></span> &bull; Jumlah: Rp <span x-text="Number(jumlah).toLocaleString('id-ID')"></span> &bull; Potongan: Rp <span x-text="Number(potongan).toLocaleString('id-ID')"></span> &bull; <strong>Tunai: Rp <span x-text="Number(diterima).toLocaleString('id-ID')"></span></strong>
                                </p>
                            </template>
                            <template x-if="!loading && found === false">
                                <p class="text-xs mt-1 text-red-600 font-medium">Kode tidak ditemukan atau sudah digunakan.</p>
